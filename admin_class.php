...
Class Action {
	...
	function save_gasto(){
		...
		$check = $this->db->query("SELECT * FROM gastos where tgasto ='$tgasto' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO gastos set $data");
		}else{
			$save = $this->db->query("UPDATE gastos set $data where id = $id");
		}
	...
	function delete_gasto(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM gastos where id = ".$id);
		if($delete){
			return 1;
		}
	}
